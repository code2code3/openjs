...
if (sessionStorage.getItem('inEU') === null) {
...
(async function main() {
     ...
     if (hasISO) {
       ...
       if (inEU === true) {
         localStorage.setItem('inEU', 'true');
       } else {
         localStorage.setItem('inEU', 'false');
       }
  ...
  if (sessionStorage.getItem('inEU') === 'true') {
   loadLimited();   
  } else {
    loadComplete();
     loadAnalytics();
   ...
   function loadComplete() {
       ...
       if (document.querySelectorAll('.the-content h2').length > 2) {
           ...
           for (let i=5, j=0; i<ps.length; i+=5, j++) {
